...
class UMaterial(Ar: FAssetArchive, exportObject: FObjectExport, validPos: Int) : UMaterialInterface(Ar, exportObject) {

    val twoSided = baseObject.getOrDefault("TwoSided", false)
    val disableDepthTest = baseObject.getOrDefault("bDisableDepthTest", false)
    val isMasked = baseObject.getOrDefault("bIsMasked", false)
    val blendMode = baseObject.getOrDefault("BlendMode", EBlendMode.BLEND_Opaque)
    val opacityMaskClipValue = baseObject.getOrDefault("OpacityMaskClipValue", 0.333f)
    val referencedTextures = baseObject.getOrDefault("ReferencedTextures", emptyArray<UTexture>(), Ar).toMutableList()
