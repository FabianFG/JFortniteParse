...
public class Package {
	...
	public String toJSON() {
		...
		for (FObjectImport oImport : importMap.getEntrys()) {
			...
			importO.put("Class Name", oImport.getClassName());
			importO.put("Class Package", oImport.getClassPackage());
			importO.put("Object Name", oImport.getObjectName());
			importMapData.add(importO);
		}
		root.put("Import Map", importMapData);
		...
		for (FObjectExport oExport : exportMap.getEntrys()) {
			...
			exportO.put("Export Type", oExport.getClassIndex().getImportName());
			exportO.put("Export Offset", oExport.getSerialOffset() - assetLength);
			exportO.put("Export Length", oExport.getSerialSize());
			exportMapData.add(exportO);
		}
		root.put("Export Map", exportMapData);
		...
		for (UObject uobject : uObjects) {
			...
			uobjectO.put("Export Type", uobject.getExportType());
			...
			uobjectO.put("Property Tags", propertyTags);
			exportProperties.add(uobjectO);
		}
		root.put("Export Properties", exportProperties);
